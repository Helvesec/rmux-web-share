...
- deriving a session from a PSK, a DH shared secret, and the exact handshake
  transcript bytes;
...
Forward secrecy comes from the ephemeral X25519 keys; authentication comes from
the PSK mixed into the key schedule. The PSK must be high-entropy: rmux uses
`SHA-256(256-bit token)`.
