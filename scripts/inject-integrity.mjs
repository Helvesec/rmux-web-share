...
// served bytes (Codex: re-hash index.html AFTER injection).
//
...
// 1. WASM pin.
const wasmFile = astroFiles.find((name) => name.endsWith('.wasm'));
if (!wasmFile) {
  throw new Error('inject-integrity: no .wasm in dist/_astro');
}
